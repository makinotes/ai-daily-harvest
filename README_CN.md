...
> **40+ AI 信源，一份结构化日报。** 评分、分类、开箱即用——人看摘要，Agent 吃 JSON，不用自己建爬虫。

每天从 40+ 中英文信源筛选 AI 内容，每篇评分 0-100，标注 verdict（`must_read` / `worth_reading` / `neutral`），拆解为关键要点、结构化推理和实践意义。自己用的，顺手公开。
...
文章元数据和 AI 分析内容遵循 [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/)。原始内容版权归原作者。
